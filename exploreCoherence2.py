...
import timeDomain_exploreCoherence
#import calc_coherence_from_time_domain
import plot_data_spectrogram
...
def calcCoherence_fromTimeDomain(data, fs, nave):
    n_per_page = 4
    nrow = int(np.ceil(n_per_page / 2))
    ncol = 2
    count = 0
    ...
    for i in range(data.shape[1]):
        ...
        f, Cxy = signal.coherence(data[:, compare_chan[i, 0] - 1], data[:, compare_chan[i, 1] - 1],
                                  fs=fs, nperseg=nave * fs)
        f = f[:len(Cxy)]
        ...
        ax.set_title('Channels {} and {}'.format(compare_chan[i, 0], compare_chan[i, 1]))
...
timeDomain_exploreCoherence.plot_data_time_domain(data_V, fs, n_per_page=8, nrow=4, ncol=2)

plot_data_spectrogram.plot_data_spectrogram(data_V, fs, t_plot_sec=None, n_per_page=4, nrow=3, ncol=1, chan_names=None,
