...
for i = 1:size(compare_chan, 1)
    ...
    % Plot spectrogram
    figure;
    surf(T, F, 10*log10(spectrogram1), 'EdgeColor', 'none');
    view(2);
    axis tight;
    colorbar;
    ...
    figure;
    surf(T, F, 10*log10(spectrogram2), 'EdgeColor', 'none');
    view(2);
    axis tight;
    colorbar;
...
%imagesc(T, F, coherence);
%pcolor(T, F, coherence);
shading interp;  % 让颜色更平滑
title('Coherence');
xlabel('Time (s)');
ylabel('Frequency (Hz)');
colorbar;
