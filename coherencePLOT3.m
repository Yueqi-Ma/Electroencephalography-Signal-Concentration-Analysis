...
for i = 1:size(compare_chan, 1)
    ...
    figure;
    %imagesc(T, F, coherence(:, i).^2, [0 1]);
    imagesc(T, F, sqrt(coherence(:, i)), [0 1]);
    set(gca, 'YDir', 'normal');
    colormap jet;
    colorbar;
    title(['Mean Square Coherence Spectrogram between Channel ' num2str(channel1) ' and Channel ' num2str(channel2)]);
    xlabel('Time (s)');
    ylabel('Frequency (Hz)');
end