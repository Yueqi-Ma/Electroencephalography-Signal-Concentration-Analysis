...
%%2015版本之后wavewrite被弃用；
%%wavwrite(foo_V(:,1:min([size(data_V,2) 2]))*1e6/500,fs_dec,16,outfname);
%audiowrite(outfname, foo_V(:,1:min([size(data_V,2) 2]))*1e6/500, fs_dec, 'BitsPerSample', 16);
normalized_data = foo_V(:,1:min([size(data_V,2) 2])) / max(abs(foo_V(:,1:min([size(data_V,2) 2]))));
audiowrite(outfname, normalized_data, fs_dec, 'BitsPerSample', 16);
...
%percentile_range = prctile(data_V, [16 84]);
%spread_data_V = diff(percentile_range)/2;
...
for Ichan=1:size(data_V,2);
    ...
    N=500;overlap = 1-1/16;plots=0;yl=[0 fs/2];
